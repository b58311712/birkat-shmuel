...
router.get('/items', asyncHandler(async (req, res) => {
  let q = supabase
    .from('inventory_items')
    .select('*, category:category_id (id, name), default_supplier:default_supplier_id (id, name)')
    .order('name');
...
router.post('/items', asyncHandler(async (req, res) => {
  const {
    ...
    default_supplier_id, last_purchase_price, is_packaging, notes,
  } = req.body || {};
  if (!name?.trim()) return fail(res, 400, 'חובה להזין שם מוצר.');
  if (!unit?.trim()) return fail(res, 400, 'חובה להזין יחידת מידה.');
...
router.patch('/items/:id', asyncHandler(async (req, res) => {
  const allowed = [
    'name', 'category_id', 'unit', 'min_alert_quantity', 'default_supplier_id',
    'last_purchase_price', 'is_packaging', 'is_active', 'notes',
  ];
  const patch = {};
  for (const k of allowed) if (k in (req.body || {})) patch[k] = req.body[k];
  if ('name' in patch && !patch.name?.trim()) return fail(res, 400, 'שם מוצר לא יכול להיות ריק.');
  if ('unit' in patch && !patch.unit?.trim()) return fail(res, 400, 'יחידת מידה לא יכולה להיות ריקה.');
