    ...
    <main className="mx-auto max-w-[1500px] px-4 py-5 sm:px-6 sm:py-7 lg:px-8 lg:py-8">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Link to="/admin/suppliers" className="btn-primary">ספקים ותשלומים</Link>
        </div>
      </header>
...
function SecondaryMetric({ label, value, tone = 'neutral' }) {
  ...
  return <div className="rounded-2xl border border-surface-line bg-white p-4 shadow-card"><p className="text-[11px] font-bold text-[#8b8084] sm:text-sm">{label}</p><p className={`mt-1 truncate text-lg font-extrabold tabular-nums sm:text-xl ${valueTone}`} dir="ltr">{nis(value)}</p></div>;
}

function ReportLink({ label, value, sub, to }) {
  return <Link to={to} className="group rounded-2xl border border-surface-line bg-[#faf9f8] p-4 transition-colors hover:border-surface-line-strong hover:bg-white"><div className="flex items-start justify-between gap-2"><span className="text-2xl font-extrabold tabular-nums text-brand-burgundy">{value ?? 0}</span><span className="text-[#b4abad] transition group-hover:-translate-x-1 group-hover:text-brand-gold-dark">←</span></div><p className="mt-1 text-sm font-bold text-[#62575b]">{label}</p>{sub && <p className="mt-0.5 text-xs font-semibold text-[#958b8e]" dir="ltr">{sub}</p>}</Link>;
}
