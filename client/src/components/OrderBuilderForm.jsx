...
import { MealCategoryPicker } from './MealCategoryPicker.jsx';
...
// גוף הטופס המשותף ליצירת הזמנה חדשה (NewOrder.jsx) ולעריכה עצמית ע"י לקוח
// (OrderEdit.jsx): בחירת סעודות/מאכלים/תוספות/פרטי משלוח, חישוב מחיר משוער,
// ותצוגה מקדימה לפני שליחה. הקומפוננטה האם אחראית על שלב בחירת השבת (אם קיים)
// ועל onSubmit - מה שקורה עם ה-payload הבנוי (יצירה מול עריכה).
//   catalog        - קטלוג מלא (חובה, כבר טעון אצל הקורא)
//   shabbatId      - מזהה השבת/המועד; משמש רק לוולידציה "יש לבחור שבת" (ביצירה
//                    זה state של ההורה; בעריכה זה תמיד קבוע ומלא מראש)
//   shabbat        - אובייקט השבת/המועד (לתצוגה בכותרת המודאל של התצוגה המקדימה)
//   customer       - הלקוח (לתצוגה בכותרת המודאל)
//   initial*       - ערכי התחלה למילוי מראש בעריכה (ברירת מחדל = טופס ריק)
//   submitLabel/confirmLabel/confirmingLabel - טקסטים הניתנים להתאמה בין יצירה לעריכה
//   onSubmit(payload) - async; payload: { slots, meals, extras, contact_name,
//                        contact_phone, venue_name, venue_address, notes, preferred_payment_method }
export function OrderBuilderForm({
  ...
  initialNotes = '',
  initialPayMethod = '',
  submitLabel = 'הצגת ההזמנה לפני שליחה',
  confirmLabel = 'אישור ושליחת ההזמנה',
  confirmingLabel = 'שולח...',
  onSubmit,
}) {
  ...
  }, [catalog, inheritByCategory, meals]);
...
function OrderPreviewModal({ customer, shabbat, preview, priceEstimate, deliveryDetails, hasException, submitting, confirmLabel, confirmingLabel, onClose, onSubmit }) {
  return (
